...
export function GET() {
  ...
  for (const page of PAGES) {
    const isHome = page === "";
    const isService = page.startsWith("/blogs/");
    const changeFreq = isHome ? "weekly" : page === "/blogs" ? "daily" : isService ? "weekly" : "monthly";
    const arPriority = isHome ? "1.0" : page === "/blogs" ? "0.85" : isService ? "0.85" : page === "/contact" ? "0.8" : "0.7";
    const enPriority = isHome ? "0.9" : page === "/blogs" ? "0.75" : isService ? "0.75" : page === "/contact" ? "0.7" : "0.6";
